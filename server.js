...
const catalogo_Schema = require("./backend/models/catalogo_Schema");
const catalogo_Model = require("./backend/models/catalogo_Schema")
const movimientos_Schema = require("./backend/models/movimientos_Schema");
...
//Metodo post para subir movimientos (general)
...
//Metodo post para subir movimientos (con datos de usuario)
app.post("/subirMovimientos/:id/:id2", upload.single('excel'), uploadMovimientos);
async function uploadMovimientos(req, res) {    
    var workbook = XLSX.read(req.file.buffer);
    var sheet_name_list = workbook.SheetNames;
    var data = XLSX.utils.sheet_to_json(workbook.Sheets[sheet_name_list[0]]);
    ...
    // Create an ID for Submission
    console.log("aver las variables: ")
    var usr = req.params.id;
    var pwd = req.params.id2;
    console.log(usr);
    var movA_Model = new mongoose.model(`excel_mov_Aux_${usr}`, movimientos_Schema, `excel_mov_Aux_${usr}`);

    await movA_Model.find().sort({"Submission_id":-1}).limit(1).then( (result) => {
        result.map(function(u){
            if (u.Submission_id == (undefined || null)) { submit_id = 0 } else { 
                submit_id = u.Submission_id + 1;
             }
            ...
            if (String(line["CONTPAQ i"]).match(/\d{3}-\d{3}/)) {
                ...
                if (line["Lecar Consultoria en TI, S.C."] == undefined  && (line["__EMPTY_3"], line["__EMPTY_4"], line["Hoja:      1"] != 0)) {
                    ...
                    */
                    ...
                    var movBaseDatos = new movA_Model ({
                        Submission_id:          submit_id,
                        Cuenta:                 VCUENTA,
                        Categoria_Total:        VCATEGORIA,
                        Fecha:                  VFECHA,
...
//Metodo post para subir catalogo de cuentas (general)
...
//Metodo post para subir catalogo de cuentas (usuario)
app.post("/subirCatalogo/:id/:id2", upload.single('excel'), uploadCatalogo);
async function uploadCatalogo(req, res) {    
    var workbook = XLSX.read(req.file.buffer);
    var sheet_name_list = workbook.SheetNames;
    var data = XLSX.utils.sheet_to_json(workbook.Sheets[sheet_name_list[0]]);
    ...
    console.log("a ver variables catalogo: ");
    var usr = req.params.id;
    var pwd = req.params.id2;
    console.log(usr, pwd);
    ...
    // Create an ID for Submission
    await catalogoA_Model.find().sort({"Submission_id":-1}).limit(1).then( (result) => {
        result.map(function(u){
            if (u.Submission_id == (undefined || null)) { submit_id = 0 } else { 
                submit_id = u.Submission_id + 1;
             }
...
//Método get para movimientos con fechas
...
//Método get para movimientos con usuario y fechas
app.get("/recibir_FechasDe_Movimientos/:id/:id2/:id3/:id4", (req, res) => {
    var dict = new Object();
    ...
    var mes1 = req.params.id;
    var mes2 = req.params.id2;
    var usr = req.params.id3;
    var pwd = req.params.id4;
...
//Método get para cuentas con usuario
app.get("/recibirCuentas/:usr/:pwd", (req, res) => {
